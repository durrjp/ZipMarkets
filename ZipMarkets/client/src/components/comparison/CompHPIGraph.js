...
export default function CompHPIGraph({twoZips}) {
    ...
    const missedYearsArray = usYearArray.map(year => {
        ...
        if(twoZips[0].hpiList.find(hpi => hpi.year === year) && twoZips[1].hpiList.find(hpi => hpi.year === year)) {
            const foundHPI1 = twoZips[0].hpiList.find(hpi => hpi.year === year)
            const foundHPI2 = twoZips[1].hpiList.find(hpi => hpi.year === year)
            yearObj["zip1"] = foundHPI1.hpi
            yearObj["zip2"] = foundHPI2.hpi
        }
        else if (twoZips[0].hpiList.find(hpi => hpi.year === year) && !twoZips[1].hpiList.find(hpi => hpi.year === year)) {
            const foundHPI1 = twoZips[0].hpiList.find(hpi => hpi.year === year)
            yearObj["zip1"] = foundHPI1.hpi
            yearObj["zip2"] = null
        }
